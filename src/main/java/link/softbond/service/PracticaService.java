...
public class PracticaService {
	...
	public Response ejecutar(Integer consultaId, String sql) {
		...
		practica.setFecha(LocalDateTime.now());
		practica.setSsql(sql);
		practica.setUsuario(usuario.getId());
	...
	private Result obtenerEjemplo(Consulta consulta) {
		try {
			this.errorExample = false;
			ResConsulta x = ConexionBD.ejecutarConsulta(consulta.getProblema().getNombrebase(), consulta.getSolucion(), 5);
			
			if(x.getRes()==null) {
				this.errorExample = true;
				this.mensaje = "La consulta de ejemplo ha presentado un error y este ejercicio no se encuentra disponible en este momento";
				this.tipo = "danger";
				return null;
			} else {
				if(x.getRes().getRowCount()<=0) {
					this.errorExample = true;
					this.mensaje = "La consulta de ejemplo no ha retornado resultados y este ejercicio no se encuentra disponible en este momento";
					this.tipo = "danger";
					return null;
				}
			}
